...
void RayCastViewer::initializeGL()
{
	...
	ldp::Float3 sv = ldp::Float3(reconstruction_dataholder.m_lights.diffuse.x,
		reconstruction_dataholder.m_lights.diffuse.y,
		reconstruction_dataholder.m_lights.diffuse.z);
	glLightfv(GL_LIGHT0, GL_DIFFUSE, sv.ptr());
	glLightfv(GL_LIGHT0, GL_AMBIENT, &reconstruction_dataholder.m_lights.amb.x);
	glLightfv(GL_LIGHT0, GL_SPECULAR, &reconstruction_dataholder.m_lights.spec.x);
...
void RayCastViewer::mouseReleaseEvent(QMouseEvent *ev)
{
		...
		&& ev->modifiers() == Qt::CTRL)
	{
		...
		float l = (width() - w1) / 2; 
		float t = (height() - h1) / 2;
		reconstruction_dataholder.m_dparam.view_click_vert_xy[0] = std::lroundf((ev->pos().x()-l)/scale);
		reconstruction_dataholder.m_dparam.view_click_vert_xy[1] = std::lroundf((ev->pos().y()-t)/scale);

...
namespace dfusion
{
	Param::Param()
	{
		...
		* tsdf volume related
		* ******************************************************/
		volume_resolution[0] = 512;
		volume_resolution[1] = 512;
		volume_resolution[2] = 512;
		voxels_per_meter = 256;

		/*volume_resolution[0] = 64;
		volume_resolution[1] = 64;
		volume_resolution[2] = 64;
		voxels_per_meter = 128;*/
		...
		* ******************************************************/
		...
		//set_warp_radius_search_epsilon(0.005);
		...
		set_warp_radius_search_epsilon(0.01);
		warp_param_softness = 0.5;
		warp_radius_search_beta = 2;
		warp_param_dw_lvup_scale = 0.01f;
		warp_point_step_before_update_node = 1;
		...
		* ******************************************************/
		...
		fusion_rigid_distThre = 0.1;// 0.1f; // meter
		fusion_rigid_ICP_iter[0] = 8; // coarse level
		fusion_rigid_ICP_iter[1] = 4;
		fusion_rigid_ICP_iter[2] = 1; // finest level
		...
		if (graph_single_level)
		{
			...
			/*fusion_lambda = 300;
			fusion_GaussNewton_fixedStep = 0.5f;
			warp_param_softness = 0.7;
			fusion_GaussNewton_diag_regTerm = 8*1e-6;
			warp_knn_k_eachlevel[1] = KnnK;*/
		}

		// save warp field and mesh to a single file
		fusion_save_warpField = true;
		fusion_save_warpField_step = 30;
		fusion_save_warpField_maxFrames = 300;
	...
	void Param::set_warp_radius_search_epsilon(float v)
	{
		...
		warp_valid_point_num_each_node = 1;//300.f * pow(v / 0.025f * 128 / 387.f, 3);

...
void drawOneLine(GLfloat x1, GLfloat y1, GLfloat z1, GLfloat x2, GLfloat y2, GLfloat z2)
{
	glBegin(GL_LINES);
	glVertex3f(x1,y1,z1);
	glVertex3f(x2,y2,z2);
...
#pragma endregion
	...
	void GpuMesh::toObjMesh(ObjMesh& omesh)
	{
		...
		for (size_t fid = 0; fid < omesh.face_list.size(); fid++)
		{
			...
			f.normal_index[0] = fid * 3;
			f.normal_index[1] = fid * 3 + 1;
			f.normal_index[2] = fid * 3 + 2;
		}
	}

	void dfusion::GpuMesh::toObjMesh_nonDup(ObjMesh & omesh)
	{
		//omesh.clear();
	...
	void dfusion::GpuMesh::formObjMesh(ObjMesh & omesh)
	{
		FILE* fp = 0;
		/*if (0 == (fp = freopen("OUT.txt", "w", stdout)))
			printf("Cannot open file.\n");*/
		int num_f = omesh.face_list.size();
		int num_v = num_f * 3;
		create(num_v);
		std::vector<PointType> tmpvert, tmpnorm;
		tmpvert.resize(num());
		...
		for (int idx_f_obj = 0; idx_f_obj < num_f; idx_f_obj++)
		{
			ObjMesh::obj_face &f = omesh.face_list[idx_f_obj];
			if (f.vertex_count != 3)
			{
				printf("dfusion::GpuMesh::formObjMesh: Not tri mesh\n");
				return;
			}
			for (int i = 0; i < 3; i++)
			{
				idx_v_obj = f.vertex_index[i];
				idx_v_gpu = 3 * idx_f_obj + i;

				Float3 v_obj = omesh.vertex_list[idx_v_obj];
				PointType v_gpu = to_point(v_obj);
				
				tmpvert[idx_v_gpu] = v_gpu;

				Float3 n_obj = omesh.vertex_normal_list[idx_v_obj];
				PointType n_gpu = to_point(n_obj);
				tmpnorm[idx_v_gpu] = n_gpu;
				//printf("idx_f_obj = %d i = %d v_obj = (%f,%f,%f)\n", idx_f_obj, i, v_obj.x(), v_obj.y(), v_obj.z());
				//printf("idx_f_obj = %d i = %d v_gpu = (%f,%f,%f,%f)\n", idx_f_obj, i, tmpvert[idx_v_gpu].x, tmpvert[idx_v_gpu].y, tmpvert[idx_v_gpu].z, tmpvert[idx_v_gpu].w);
			}
		}
		lockVertsNormals();
		cudaMemcpy(verts(), tmpvert.data(), num() * sizeof(PointType), cudaMemcpyHostToDevice);
		cudaMemcpy(normals(), tmpnorm.data(), num() * sizeof(PointType), cudaMemcpyHostToDevice);
		unlockVertsNormals();
		//fclose(fp);
	}

	void dfusion::GpuMesh::formObjMesh_nonDup(ObjMesh & omesh)
	{

		int num_v = omesh.vertex_list.size();
		create(num_v);
		std::vector<PointType> tmpvert, tmpnorm;
		tmpvert.resize(num());
		tmpnorm.resize(num());
		...
		for (idx_v_obj = 0; idx_v_obj < num_v; idx_v_obj++)
		{
			//printf("formObjMesh_nonDup : idx_v_obj = %d\n", idx_v_obj);
			Float3 v_obj = omesh.vertex_list[idx_v_obj];
			PointType v_gpu = to_point(v_obj);
			tmpvert[idx_v_obj] = v_gpu;
			Float3 n_obj = omesh.vertex_normal_list[idx_v_obj];
			PointType n_gpu = to_point(n_obj);
			tmpnorm[idx_v_obj] = n_gpu;
		}
		lockVertsNormals();
		cudaMemcpy(verts(), tmpvert.data(), num() * sizeof(PointType), cudaMemcpyHostToDevice);
		cudaMemcpy(normals(), tmpnorm.data(), num() * sizeof(PointType), cudaMemcpyHostToDevice);
		...
		if (warpField && (param.view_show_nodes || param.view_show_graph))
		{
			...
			if (param.view_show_graph)
			{
				...
				glVertexPointer(3, GL_FLOAT, sizeof(float4),
					...
					WarpField::MaxNodeNum * 2 * KnnK*sizeof(float4)* param.view_show_graph_level));
				glColorPointer(3, GL_FLOAT, sizeof(float4),
					(void*)(WarpField::MaxNodeNum*WarpField::GraphLevelNum*sizeof(float4)+
					WarpField::MaxNodeNum * 2 * KnnK*sizeof(float4)* WarpField::GraphLevelNum+
					WarpField::MaxNodeNum * 2 * KnnK*sizeof(float4)* param.view_show_graph_level));
				glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_vbo_id_warpnodes);
				glColor3fv(colors[param.view_show_graph_level].ptr());
				glDrawArrays(GL_LINES, 0, warpField->getNumNodesInLevel(param.view_show_graph_level)
					*KnnK * 2);
	...
	void dfusion::GpuMesh::renderToImgOnlyMesh(const Camera & camera, LightSource light,
		ColorMap & img, const Param & param, const MapArr * vmap_live,
		const MapArr * vmap_warp, const MapArr * nmap_live, const MapArr * nmap_warp,
		GpuMesh * canoMesh, const float3 * canoPosActive,
		const KnnIdx * knnIdxActiveView, const Intr * intr, Tbx::Transfo *tr,
		float3* volume_pos,float3* volume_size
	)
	{
		if (!wglMakeCurrent(g_hdc, g_glrc))
			throw std::exception("wglMakeCurrent error");
		...
		if (tr)
		{
			Tbx::Transfo T = tr->transpose();
			glMultMatrixf(&T[0]);
		}
		if (volume_pos && volume_size)
		{
			glPushMatrix();
			glTranslatef(volume_pos->x, volume_pos->y, volume_pos->z);
			glScalef(volume_size->x/2, volume_size->y/2, volume_size->z/2);
			glColor3f(0.5, 0.5, 0.5);
			glLineWidth(3);

			drawOneLine(-1, -1, 0, 1, -1, 0);
			drawOneLine(1, -1, 0, 1, 1, 0);
			drawOneLine(1, 1, 0, -1, 1, 0);
			drawOneLine(-1, 1, 0, -1, -1, 0);

			drawOneLine(-1, -1, 2, 1, -1, 2);
			drawOneLine(1, -1, 2, 1, 1, 2);
			drawOneLine(1, 1, 2, -1, 1, 2);
			drawOneLine(-1, 1, 2, -1, -1, 2);

			drawOneLine(-1, -1, 2, -1, -1, 0);
			drawOneLine(1, -1, 2, 1, -1, 0);
			drawOneLine(1, 1, 2, 1, 1, 0);
			drawOneLine(-1, 1, 2, -1, 1, 0);
			glPopMatrix();
	...
	void GpuMesh::renderToImg(const Camera& camera, LightSource light, ColorMap& img, float3 volume_pos, float3 volume_size,
		const Param& param, const WarpField* warpField,
		const MapArr* vmap_live, const MapArr* vmap_warp,
		const MapArr* nmap_live, const MapArr* nmap_warp,
		GpuMesh* canoMesh,
		const float3* canoPosActive,
		const KnnIdx* knnIdxActiveView,
		const Intr* intr, bool warp_nodes, bool norigid)
	{
		if (!wglMakeCurrent(g_hdc, g_glrc))
			throw std::exception("wglMakeCurrent error");
		if (vmap_live || vmap_warp)
		...
		camera.apply();
		//glPushMatrix();
		////Tbx::Vec3 translate = rigid.get_translation();

		////glTranslatef(translate.x, translate.y, translate.z);

		////glMultMatrixf((GLfloat*)&rigid.full_invert());
		////glMultMatrixf((GLfloat*)&rigid);
		//glTranslatef(volume_pos.x, volume_pos.y, volume_pos.z);
		////glTranslatef(volume_size.x/2, volume_size.y/2, volume_size.z/2);
		//glScalef(volume_size.x, volume_size.y, volume_size.z);
		////glTranslatef(0, 0, -1);
		//glColor3f(0.5, 0.5, 0.5);
		//glLineWidth(3);
		//glutWireCube(1);
	...
	void GpuMesh::renderToDepth(const Camera& camera, DepthMap& img, Tbx::Transfo *tr)
	{
		if (!wglMakeCurrent(g_hdc, g_glrc))
			throw std::exception("wglMakeCurrent error");
		const int width = std::lroundf(abs(camera.getViewPortRight() - camera.getViewPortLeft()));
		...
		if (tr)
		{
			Tbx::Transfo T = tr->fast_invert().transpose();
			glMultMatrixf(&T[0]);
		}

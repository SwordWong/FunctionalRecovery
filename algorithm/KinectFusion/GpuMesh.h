...
namespace dfusion
{
	...
	* *************************************************************************/
	...
	public:
		...
		__device__ __host__ __forceinline__ static GpuMesh::PointType to_point(ldp::Float3 p, float w = 1.f)
		{
	...
	public:
		...
		void toObjMesh_nonDup(ObjMesh& omesh);
		void formObjMesh(ObjMesh& omesh);
		void formObjMesh_nonDup(ObjMesh& omesh);

		// unlockVertsNormals is performed insided
		void renderToImg(const Camera& camera, LightSource light, ColorMap& img, float3 volume_pos, float3 volume_size,
		...
		void renderToImgOnlyMesh(const Camera& camera, LightSource light, ColorMap& img,
			const Param& param,
			const MapArr* vmap_live = nullptr, const MapArr* vmap_warp = nullptr,
			const MapArr* nmap_live = nullptr, const MapArr* nmap_warp = nullptr,
			GpuMesh* canoMesh = nullptr,
			...
			Tbx::Transfo *tr = nullptr,
			float3* volume_pos = nullptr,
			float3* volume_size = nullptr
		);
		void renderToDepth(const Camera& camera, DepthMap& img, Tbx::Transfo *tr = nullptr);

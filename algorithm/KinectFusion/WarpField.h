...
namespace dfusion
{
	...
	public:
		...
		void init(Param param, WarpVolumeParam wvParam);
		void clear();
		...
		void warp(ObjMesh& src, ObjMesh& dst);
		...
		WarpVolumeParam getWarpVolumeParam()const { return warp_volume_param; }
		...
		// for each vertex, extract its knn idx
		void extract_knnIdx_for_verts(GpuMesh& src, DeviceArray<KnnIdx>& knns);

		// export some data to file
		void export_vertKnn_nodes_graph(GpuMesh& src, FILE* pFile);
	protected:
		void initKnnField();
	...
	private:
		...
		WarpVolumeParam warp_volume_param;
